...
!
! Change Log ------------------------------------------------------------------------------------------------
! PJW 05/08/2025: Copied from niva/nersem/nersem_carbonate.F90, renamed modules.
...
!                 Removed conversion of per-day to per-second using self%dt and modified units
!                 conversion of fair accordingly.
...
! PJW 07/05/2026: Reinstated subroutines do and CaCO3_Saturation since we now need Om_cal
!                 calculated throughout the water column to define added calcite dynamics in niva_ecosmo
!                 (old code renamed to niva_ecosmo_carbonate_nocalc).
! -----------------------------------------------------------------------------------------------------------


module niva_ecosmo_carbonate
   ...
   type,extends(type_base_model),public :: type_niva_ecosmo_carbonate
...
!
! !INPUT PARAMETERS:
      class (type_niva_ecosmo_carbonate), intent(inout), target :: self
...
!BOC
   ...
   subroutine do(self,_ARGUMENTS_DO_)
      class (type_niva_ecosmo_carbonate), intent(in) :: self
      _DECLARE_ARGUMENTS_DO_

      real(rk) :: dic,ETW,X1X,density,pres
      real(rk) :: TA,Ctot
      ...
      _LOOP_BEGIN_
         ...
         X1X = max(X1X, 0.0_rk) ! Inserted PWA 06/01/2020
   ...
   subroutine do_surface(self,_ARGUMENTS_DO_SURFACE_)
      ...
      _HORIZONTAL_LOOP_BEGIN_
         ...
         S = max(S, 0.0_rk) ! Inserted PWA 06/01/2020
         wnd = max(wnd, 0.0_rk)
