...
! It is the 'nocalc' version because, as in the version of ecosmo_operational.F90 that is 
! was derived from, the effects on calcite formation/dissolution are not accounted for and 
! there is no detrtical calcite state variable).
! -----------------------------------------------------------------------------------------------------------
!
!
! Change Log ------------------------------------------------------------------------------------------------
...
!                    significant for metazoan predators (Steinberg and Landry, 2017, although MEDUSA (Yool et al., 2015)
...
!                 8) The source-independent dissolved fraction of organic waste parameter frr was split into a set of parameters
!                    allowing different fractions for different mortality/egestion sources (frmort1Ps etc.).
...
!                    a) These fluxes are thought to have highly non-Redfield stoichiometry (e.g. in ERSEM, only the carbon productivity
!                       is corrected, so only DOC is produced). Included them within the Redfield stoichiometry
...
! PJW 07/05/2026: Renamed niva_ecosmo->niva_ecosmo_nocalc. 
! -----------------------------------------------------------------------------------------------------------

   module niva_ecosmo_nocalc
!
! !DESCRIPTION:
!
...
! !PUBLIC MEMBER FUNCTIONS:
   public type_niva_ecosmo_nocalc
!
...
! !PUBLIC DERIVED TYPES:
   type,extends(type_base_model) :: type_niva_ecosmo_nocalc
!     Variable identifiers
      ...
      type (type_dependency_id)             :: id_temp, id_salt, id_E0
...
!     Model procedures
   ...
   end type type_niva_ecosmo_nocalc
!EOP
!-----------------------------------------------------------------------
...
!
! !INPUT PARAMETERS:
   class (type_niva_ecosmo_nocalc),intent(inout),target  :: self
...
!
! !INPUT PARAMETERS:
   class (type_niva_ecosmo_nocalc),intent(in) :: self
...
! !LOCAL VARIABLES:
   ...
   real(rk) :: rhs,rhs_nit,rhs_amm,rhs_pho,rhs_oxy
!EOP
!-----------------------------------------------------------------------
!BOC
...
! reaction rates
   ...
   ! carbonate dynamics
   if (self%couple_co2) then
     _ADD_SOURCE_(self%id_dic, redf(16)*(excrmin + remtot - Prod))
...
!
! !LOCAL VARIABLES:
   real(rk) :: temp, oxy, no3, det, opa, sed1, sed2, sed3
   real(rk) :: tbs
   real(rk) :: Rds, RdsO, Rsd
...
!BOC
   ...
   if (self%couple_co2) then
     _ADD_BOTTOM_FLUX_(self%id_dic, redf(16)*remSED)
